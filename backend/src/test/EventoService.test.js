...
describe('EventoService', () => {
    ...
    describe('criarEvento', () => {
        ...
        it('deve criar um evento com sucesso com os parâmetros obrigatórios', async () => {
            ...
            const resultado = await EventoService.criarEvento(nome, local, data, descricao);
        ...
        it('deve lançar um erro se algum campo obrigatório estiver faltando', async () => {
            await expect(EventoService.criarEvento(null, local, data, descricao)).rejects.toThrow(
                'Todos os campos são obrigatórios'
            );

            await expect(EventoService.criarEvento(nome, local, null, descricao)).rejects.toThrow(
                'Todos os campos são obrigatórios'
            );

            expect(mockEventoRepository.obterPorNomeLocalEData).not.toHaveBeenCalled();
        });
        ...
        it('deve lançar um erro se já existir um evento duplicado', async () => {
            mockEventoRepository.obterPorNomeLocalEData.mockResolvedValue(eventoCriadoMock);

            await expect(EventoService.criarEvento(nome, local, data, descricao)).rejects.toThrow(
    ...
    describe('obterPorId', () => {
        ...
        it('deve lançar um erro se o evento não for encontrado', async () => {
            ...
            await expect(EventoService.obterPorId(999)).rejects.toThrow('Evento não encontrado');
        });
    });
    ...
    describe('obterPorNome', () => {
        ...
        it('deve retornar o evento se ele for encontrado', async () => {
            ...
            expect(resultado).toEqual(eventoMock);
            expect(mockEventoRepository.obterPorNome).toHaveBeenCalledWith('Show Teste');
        });

        it('deve lançar um erro se o evento não for encontrado', async () => {
            mockEventoRepository.obterPorNome.mockResolvedValue(null);

            await expect(EventoService.obterPorNome('Show Inexistente')).rejects.toThrow(
    ...
    describe('obterPorLocal', () => {
        ...
        it('deve retornar o evento se ele for encontrado', async () => {
            ...
            expect(resultado).toEqual(eventoMock);
            expect(mockEventoRepository.obterPorLocal).toHaveBeenCalledWith('Parque Central');
        });

        it('deve lançar um erro se o evento não for encontrado', async () => {
            mockEventoRepository.obterPorLocal.mockResolvedValue(null);

            await expect(EventoService.obterPorLocal('Local Inexistente')).rejects.toThrow(
                'Evento não encontrado'
            );
        });
    });
