...
describe('ReporteService', () => {
    ...
    describe('criarOuConfirmarReporte', () => {
        ...
        it('deve lançar erro se reportanteId ou eventoInput estiverem faltando', async () => {
            await expect(reporteService.criarOuConfirmarReporte(null, eventoInput)).rejects.toThrow(
                'Reportante e evento são obrigatórios para criar um reporte!'
            );
            await expect(
                reporteService.criarOuConfirmarReporte(reportanteId, null)
            ).rejects.toThrow('Reportante e evento são obrigatórios para criar um reporte!');
        });
        ...
        it('deve criar um novo Evento e um novo Reporte se o evento não for encontrado', async () => {
            ...
            mockPrismaClient.reporteEvento.create.mockResolvedValue(novoReporteCriado);
            ...
            expect(novoEventoCriado.setReportadoPorUsuario).toHaveBeenCalledWith(true);
        ...
        it('deve confirmar um reporte existente e incrementar qtdConfirmacoes', async () => {
            ...
            mockPrismaClient.reporteEvento.findFirst.mockResolvedValue(reporteParaAtualizar);

            const reporteAtualizadoMock = {
                ...reporteParaAtualizar,
                qtdConfirmacoes: 6,
                reportantes: [...reporteParaAtualizar.reportantes, { id: reportanteId }],
            };
            mockPrismaClient.reporteEvento.update.mockResolvedValue(reporteAtualizadoMock);
            ...
            expect(resultado.mensagem).toBe('Confirmação adicionada a um evento já reportado.');
        ...
        it('deve retornar uma mensagem se o usuário já confirmou o reporte existente', async () => {
            ...
            mockPrismaClient.reporteEvento.findFirst.mockResolvedValue(reporteJaConfirmado);
            ...
            expect(resultado.mensagem).toBe('Usuário já reportou/confirmou este evento.');
        ...
        it('deve lançar erro se o usuário tentou reportar o mesmo evento recentemente', async () => {
            ...
            ).rejects.toThrow('Você já enviou um reporte recentemente. Aguarde alguns minutos.');
    ...
    describe('listarPorUsuario', () => {
        it('deve chamar prisma.reporteEvento.findMany com os filtros OR corretos', async () => {
            ...
            expect(mockPrismaClient.reporteEvento.findMany).toHaveBeenCalledWith({
                where: {
                    OR: [{ reportanteId: usuarioId }, { reportantes: { some: { id: usuarioId } } }],
                },
                include: { evento: true, reportantes: true },
            });
            expect(resultado).toEqual(reportesMock);
    ...
    describe('removerReporte', () => {
        ...
        it('deve remover o reporte e retornar true se ele existir', async () => {
            ...
            expect(mockReporteEventoRepository.buscarPorId).toHaveBeenCalledWith(reporteId);
            expect(mockReporteEventoRepository.remover).toHaveBeenCalledWith(reporteId);
    ...
    describe('listarEventosMaisReportados', () => {
        it('deve retornar eventos ordenados por total de confirmações nas últimas 24h', async () => {
            const vinteQuatroHorasAtras = new Date(MOCK_TIME.getTime() - 24 * 60 * 60 * 1000);
            ...
            mockPrismaClient.reporteEvento.findMany.mockResolvedValue(reportesRecentesMock);
