...
describe('CaravanaService', () => {
    ...
    describe('criarCaravana', () => {
        it('deve criar uma caravana com sucesso com apenas o nome', async () => {
            ...
            expect(mockCaravanaRepository.criarCaravana).toHaveBeenCalledWith(nome, null, 10);
        });
    ...
    describe('adicionarMembro', () => {
        ...
        it('deve adicionar o membro se a caravana existir e o membro não estiver presente', async () => {
            ...
            mockCaravanaRepository.obterPorId.mockResolvedValueOnce(caravanaComMembroMock);
            mockCaravanaRepository.adicionarMembro.mockResolvedValue(true);

            const resultado = await CaravanaService.adicionarMembro(caravanaId, usuarioId);
        ...
        it('deve lançar um erro se a caravana não for encontrada', async () => {
            ...
            await expect(CaravanaService.adicionarMembro(999, usuarioId)).rejects.toThrow(
                'Grupo não encontrado.'
            );
        });

        it('deve lançar um erro se o usuário já for membro da caravana', async () => {
            mockCaravanaRepository.obterPorId.mockResolvedValue(caravanaComMembroMock);

            await expect(CaravanaService.adicionarMembro(caravanaId, usuarioId)).rejects.toThrow(
                'Usuário já faz parte da caravana.'
            );
    ...
    describe('adicionarDestino', () => {
        it('deve chamar o repositório para adicionar o destino', async () => {
            ...
            expect(mockCaravanaRepository.adicionarDestino).toHaveBeenCalledWith(1, 5);
        });
    });
    ...
    describe('calcularMultiplicador', () => {
        ...
        it('deve usar o mês e ano atuais se não forem fornecidos', async () => {
            ...
            const startCurrent = new Date(mockDate.getFullYear(), mockDate.getMonth(), 1);
            const endCurrent = new Date(mockDate.getFullYear(), mockDate.getMonth() + 1, 1);
