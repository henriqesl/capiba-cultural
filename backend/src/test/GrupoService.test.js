...
describe('GrupoService', () => {
    ...
    describe('criarGrupo', () => {
        ...
        it('deve criar um grupo com sucesso com valores padrão', async () => {
            mockGrupoRepository.novoGrupoCompeticao.mockResolvedValue(grupoCriadoMock);

            const resultado = await grupoService.criarGrupo(nome, dataInicio, dataFim);
        ...
        it('deve lançar um erro se o nome for nulo', async () => {
            await expect(grupoService.criarGrupo(null, dataInicio, dataFim)).rejects.toThrow(
        ...
        it('deve lançar um erro se as datas de início ou fim estiverem faltando', async () => {
            await expect(grupoService.criarGrupo(nome, null, dataFim)).rejects.toThrow(
                'Datas de início e fim são obrigatórias'
            );

            await expect(grupoService.criarGrupo(nome, dataInicio, null)).rejects.toThrow(
                'Datas de início e fim são obrigatórias'
            );
        });
    });
    ...
    describe('obterPorId', () => {
        ...
        it('deve lançar um erro se o grupo não for encontrado', async () => {
            ...
            await expect(grupoService.obterPorId(999)).rejects.toThrow('Grupo não encontrado.');
        });
    });
    ...
    describe('atualizarPontuacao', () => {
        ...
        it('deve calcular pontuação zero se não houver membros', async () => {
            ...
            expect(mockGrupoRepository.setPontuacaoTotal).toHaveBeenCalledWith(grupoId, 0);
        });
    });
    ...
    describe('adicionarMembro', () => {
        ...
        it('deve adicionar o membro, atualizar a pontuação e retornar o grupo atualizado', async () => {
            ...
            expect(mockGrupoRepository.adicionarMembro).toHaveBeenCalledWith(grupoId, usuarioId);
        ...
        it('deve lançar erro se o grupo não for encontrado', async () => {
            ...
            await expect(grupoService.adicionarMembro(999, usuarioId)).rejects.toThrow(
        ...
        it('deve lançar erro se o grupo estiver encerrado', async () => {
            ...
            await expect(grupoService.adicionarMembro(grupoId, usuarioId)).rejects.toThrow(
    ...
    describe('encerrarGrupo', () => {
        ...
        it('deve encerrar o grupo, setar o vencedor e retornar o status final', async () => {
            ...
            expect(mockGrupoRepository.setEncerrado).toHaveBeenCalledWith(grupoId, true);
            expect(mockGrupoRepository.setVencedor).toHaveBeenCalledWith(grupoId, vencedorMock.id);
        ...
        it('deve setar o vencedor como null se não houver membros', async () => {
            ...
            expect(mockGrupoRepository.setVencedor).toHaveBeenCalledWith(grupoId, null);
    ...
    describe('verificarGruposExpirados', () => {
        ...
        it('deve encerrar apenas grupos que não estão encerrados e cuja dataFim já passou', async () => {
            mockGrupoRepository.listar.mockResolvedValue([grupoExpirado, grupoAtivo, grupoFechado]);
