...
const MAPBOX_ACCESS_TOKEN = 'pk.eyJ1IjoiZ3VpbW9udGVuZWdybyIsImEiOiJjbWo0d2JyaGswYXN1M2hxMjZ2ejN2dGoyIn0.fxxtwS3S493xLPud44zD3A'; 
...
export const geocodeLocation = async (locationName) => {
    ...
    try {
        ...
        // Usando o Geocoding para encontrar um local
        ...
        if (data.features && data.features.length > 0) {
            const [longitude, latitude] = data.features[0].center; // Mapbox retorna [longitude, latitude]
...
export const reverseGeocodeLocation = async (latitude, longitude) => {
    try {
        ...
        if (data.features && data.features.length > 0) {
            // Retorna o nome formatado completo do local
