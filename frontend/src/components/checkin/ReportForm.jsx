                ...
                <form className="p-6 md:p-8 space-y-6" onSubmit={handleSubmit}>
                        ...
                        {isCameraActive ? (
                            <div className="relative w-full rounded-xl overflow-hidden shadow-lg bg-gray-900">
                                ...
                                <div className="absolute inset-x-0 bottom-0 p-4 flex justify-center gap-4 bg-black/30">
                                    <button type="button" onClick={handleCapture} className="bg-red-500 text-white p-4 rounded-full shadow-xl hover:bg-red-600"><Camera className="w-8 h-8" /></button>
                                    <button type="button" onClick={() => setIsCameraActive(false)} className="bg-gray-500 text-white p-4 rounded-full shadow-xl hover:bg-gray-600"><StopCircle className="w-8 h-8" /></button>
                                </div>
                            </div>
                        ) : (
                                ...
                                {imagePreview ? (
                                    <div className="relative w-full h-full">
                                        ...
                                        <div className="absolute inset-0 bg-black/30 flex items-center justify-center opacity-0 hover:opacity-100 transition-opacity z-10 pointer-events-none">
                                            <p className="text-white font-bold bg-red-600 px-4 py-2 rounded-full flex items-center gap-2"><Camera className="w-5 h-5" /> Imagem Capturada</p>
                                        </div>
                                    </div>
                                ) : (
                                    <div className="flex flex-col sm:flex-row items-center justify-center gap-4 p-4 text-red-600">
                                        <button type="button" onClick={() => setIsCameraActive(true)} className="flex flex-col items-center justify-center p-4 bg-red-100 rounded-xl hover:bg-red-200 transition-colors shadow">
                    ...
                    <div>
                        ...
                        <div className="flex flex-col sm:flex-row gap-2">
                            <input value={local} onChange={e => {setLocal(e.target.value); setCoords(null);}} type="text" placeholder="Endereço..." className="flex-1 p-2.5 border border-gray-300 rounded-lg outline-none" required />
                            <button type="button" onClick={handleCurrentLocation} disabled={isGeolocating || isGeocoding} className="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2.5 rounded-lg flex items-center justify-center gap-1">
                                {isGeolocating ? <Loader2 className="w-5 h-5 animate-spin" /> : <MapPin className="w-5 h-5" />} GPS
                            </button>
                            <button type="button" onClick={geocodeAndDisplayMap} disabled={isGeocoding || !local} className="bg-red-500 hover:bg-red-600 text-white px-4 py-2.5 rounded-lg flex items-center justify-center gap-1">
                    ...
                    <button type="submit" className={`w-full text-white font-bold py-4 rounded-xl shadow-lg flex items-center justify-center gap-2 ${isLoading || isCameraActive ? 'bg-gray-500 cursor-not-allowed' : 'bg-red-600 hover:bg-red-700'}`} disabled={isLoading || isCameraActive}>
                        {isLoading ? <><Loader2 className="w-6 h-6 animate-spin" /> Enviando...</> : <><CheckCircle className="w-6 h-6" /> Enviar Reporte</>}
                    </button>
                </form>
            </div>
        </div>
