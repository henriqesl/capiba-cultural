...
// URL do seu servidor para carregar as fotos
...
const EventMap = ({ events = [] }) => {
    ...
    // 1. Estilos dos Marcadores e Popups
    useEffect(() => {
    ...
    // 2. Inicializar Mapa (COLORIDO + INCLINADO)
    useEffect(() => {
        if (!map.current && mapContainer.current) {
            map.current = new mapboxgl.Map({
                container: mapContainer.current,
                style: 'mapbox://styles/mapbox/streets-v11', // MAPA COLORIDO
    ...
    // 3. Processar Coordenadas dos Eventos
    useEffect(() => {
        const processMarkers = async () => {
            const safeEvents = Array.isArray(events) ? events : [];
            const coordsPromises = safeEvents.map(async (evento) => {
                if (evento.latitude && evento.longitude) {
                    return { ...evento, latitude: parseFloat(evento.latitude), longitude: parseFloat(evento.longitude) };
                }
                if (evento.local) {
    ...
    // 4. Renderizar Marcadores e Popups com Fotos
    useEffect(() => {
        ...
        markersData.forEach(evento => {
            ...
            const rotaDestino = isUserEvent ? `#/eventos/${evento.id}` : `#/locais/${evento.id}/mapa`;

            // Tratamento Dinâmico da Imagem
            const imgUrl = evento.imagemUrl 
                ? (evento.imagemUrl.startsWith('http') ? evento.imagemUrl : `${API_URL}/${evento.imagemUrl}`)
            ...
            const marker = new mapboxgl.Marker({ element: el, anchor: 'bottom' })
                ...
                .setPopup(new mapboxgl.Popup({ offset: 42, closeButton: false }).setHTML(popupHTML))
                .addTo(map.current);
