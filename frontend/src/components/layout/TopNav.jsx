...
import { Calendar, CircleStar, Star, User, LogOut } from "lucide-react";
import { useAuth } from "../../context/AuthContext";

const NavItem = ({ href, icon: Icon, label, active = false }) => {
  const activeClass = active
    ? "text-white border-b-2 border-white"
    : "text-blue-200 border-b-2 border-transparent hover:text-white";

  return (
    <a
      href={href}
      className={`flex items-center gap-2 py-2 px-3 transition-all duration-200 ${activeClass}`}
    >
