...
import { Trophy, Star, CheckCircle, TrendingUp, Wallet, Target } from 'lucide-react';
...
const StatusPage = () => {
    ...
    if (loading) {
        return <div className="p-8 text-center text-gray-500">Carregando suas conquistas...</div>;
    }
        ...
        <div className="p-4 md:p-8 space-y-6 bg-gray-50 min-h-screen pb-24">
            ...
            <div className="bg-gradient-to-r from-blue-600 to-blue-800 rounded-3xl p-6 text-white shadow-lg flex justify-between items-center relative overflow-hidden">
                <div className="relative z-10">
                    <p className="text-blue-200 text-sm font-medium mb-1">Seu Saldo Atual</p>
                    <h1 className="text-4xl font-bold flex items-center gap-2">
                        {saldo} <span className="text-2xl font-normal opacity-80">Capibas</span>
                    </h1>
                </div>
            ...
            <div>
                ...
                <div className="space-y-3">
                    {missoes.length > 0 ? (
                        missoes.map((missao) => (
                            <div key={missao.id} className="bg-white p-4 rounded-xl shadow-sm border border-gray-100 relative overflow-hidden">
                                <div className="flex justify-between items-start mb-2">
                                    ...
                                    <span className="bg-yellow-100 text-yellow-700 text-xs font-bold px-2 py-1 rounded-lg flex items-center gap-1">
                                        +{missao.recompensa} <Star className="w-3 h-3 fill-current" />
                                    </span>
                                </div>
                                ...
                                <div className="w-full bg-gray-100 rounded-full h-2.5 mt-3">
                                    <div 
                                        ...
                                        style={{ width: `${Math.min((missao.progressoAtual / missao.meta) * 100, 100)}%` }}
                                    ></div>
                                </div>
                                <div className="flex justify-between text-xs mt-1 text-gray-500 font-medium">
                                    <span>{missao.progressoAtual} / {missao.meta}</span>
                                    {missao.completada && <span className="text-green-600 font-bold">Concluída!</span>}
                                </div>
                            </div>
                        ))
                    ) : (
            ...
            <div>
                ...
                <div className="bg-white rounded-xl shadow-sm border border-gray-100 divide-y divide-gray-100">
                    {historico.length > 0 ? (
                        historico.slice(0, 5).map((checkin) => (
                            <ConfirmedEventRow 
                                key={checkin.id} 
