...
const AdminPage = () => {
  ...
  const carregarDados = () => {
    if (activeTab === "dashboard" || activeTab === "aprovacoes")
      fetchPendentes();
    if (activeTab === "dashboard" || activeTab === "gestao_eventos")
      fetchEventosAtivos();
  };

  const fetchPendentes = async () => {
    try {
      const res = await api.get("/eventos?pendentes=true");
  ...
  const fetchEventosAtivos = async () => {
    try {
      ...
      const res = await api.get(
        `/eventos?mes=${hoje.getMonth() + 1}&ano=${hoje.getFullYear()}`,
      );
          ...
          <div className="animate-in fade-in space-y-6">
            <div className="grid grid-cols-2 gap-4">
              <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm text-center">
                <span className="p-3 bg-orange-100 text-orange-600 rounded-full mb-2 inline-block">
                  <Clock size={24} />
                </span>
              ...
              <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm text-center">
                <span className="p-3 bg-blue-100 text-blue-600 rounded-full mb-2 inline-block">
                  <Calendar size={24} />
                </span>
                <h3 className="text-3xl font-black text-gray-800">
          ...
          <div className="space-y-4 animate-in slide-in-from-bottom-2">
            ...
            {getPendentesFiltrados().length === 0 ? (
              <div className="text-center py-12 bg-white rounded-2xl border border-dashed border-gray-200">
                <p className="text-gray-400 font-medium">Tudo limpo!</p>
              </div>
            ) : (
              getPendentesFiltrados().map((evt) => {
                ...
                return (
                  <div
                    key={evt.id}
                    className="bg-white p-4 rounded-2xl shadow-sm border border-gray-100 flex flex-col sm:flex-row gap-4"
                  >
                    <div className="w-full sm:w-24 h-24 bg-gray-100 rounded-xl overflow-hidden shrink-0 relative">
                      {evt.imagemUrl ? (
                        <img
                          src={getFullImageUrl(evt.imagemUrl)}
                      ...
                      <div
                        className={`absolute top-0 inset-x-0 h-1 ${isReporte ? "bg-red-500" : "bg-blue-500"}`}
                      />
                    </div>
                    <div className="flex-1">
                      <div className="flex justify-between items-start">
                        ...
                        <span
                          className={`text-[10px] font-bold px-1.5 py-0.5 rounded ${isReporte ? "bg-red-100 text-red-600" : "bg-blue-100 text-blue-600"}`}
                        >
                          {isReporte ? "REPORTE" : "SUGESTÃO"}
                        </span>
                      </div>
                      <p className="text-xs text-gray-500 flex items-center gap-1 mt-1">
                        <MapPin size={12} /> {evt.local}
                      </p>
                      ...
                      <div className="flex gap-2 mt-3">
                        <button
                          onClick={() => handleAprovar(evt.id)}
                          className="flex-1 py-2 bg-green-50 text-green-700 hover:bg-green-100 rounded-lg text-xs font-bold"
                        >
                          Aprovar
                        </button>
                        <button
                          onClick={() => handleRejeitar(evt.id)}
                          className="flex-1 py-2 bg-red-50 text-red-700 hover:bg-red-100 rounded-lg text-xs font-bold"
                        >
                          Rejeitar
                        </button>
                      </div>
          ...
          <div className="space-y-6 animate-in slide-in-from-right-2">
            ...
            <div className="bg-white p-5 rounded-2xl shadow-sm border border-gray-100">
              <h2 className="text-sm font-bold text-gray-800 mb-4 flex items-center gap-2">
                <Plus size={16} className="text-blue-600" /> Novo Evento Oficial
              </h2>
              <form
                onSubmit={handleCriarEventoOficial}
                className="grid grid-cols-2 gap-3"
              >
                <input
                  className="col-span-2 p-2.5 bg-gray-50 rounded-lg text-sm border outline-none focus:border-blue-500 font-bold"
                ...
                />
                <input
                  className="p-2.5 bg-gray-50 rounded-lg text-sm border outline-none focus:border-blue-500"
                  placeholder="Local"
                ...
                />
                <input
                  className="col-span-2 p-2.5 bg-gray-50 rounded-lg text-sm border outline-none focus:border-blue-500"
                  placeholder="Horário (Ex: 19h)"
                ...
                <textarea
                  rows="2"
                  className="col-span-2 p-2.5 bg-gray-50 rounded-lg text-sm border outline-none focus:border-blue-500 resize-none"
                  ...
                  required
                />
                <button className="col-span-2 py-3 bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-lg text-sm transition-colors">
                  Publicar na Agenda
                </button>
              </form>
            </div>
            ...
            <div className="space-y-2">
              ...
              {eventosAtivos.map((evt) => (
                <div
                  key={evt.id}
                  className="bg-white p-3 rounded-xl border border-gray-100 flex justify-between items-center"
                >
        ...
        {activeTab === "gestao_missoes" && (
          <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 animate-in slide-in-from-right-2">
            ...
            <form onSubmit={handleCriarMissao} className="space-y-4">
              <input
                className="w-full p-3 bg-gray-50 rounded-xl font-bold border outline-none focus:border-purple-500"
              ...
              />
              <textarea
                className="w-full p-3 bg-gray-50 rounded-xl text-sm border outline-none focus:border-purple-500 resize-none"
                rows="3"
                placeholder="Descrição"
              ...
              <input
                type="number"
                className="w-full p-3 bg-yellow-50 text-yellow-700 rounded-xl font-bold outline-none border border-yellow-100"
                placeholder="Recompensa (Capibas)"
                value={novaMissao.recompensaCapibas}
                onChange={(e) =>
                  setNovaMissao({
                    ...novaMissao,
                    recompensaCapibas: e.target.value,
                  })
                }
              />
              <button
                type="submit"
                className="w-full py-4 bg-gray-900 hover:bg-black text-white font-bold rounded-xl transition-all shadow-lg mt-2"
              >
                Lançar Missão
              </button>
            </form>
