...
const ProfilePage = () => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      alert(
        "Erro ao atualizar. Verifique se preencheu os campos obrigatórios.",
      );
    } finally {
      setLoading(false);
    }
    ...
    <div className="w-full min-h-screen bg-gray-50 flex flex-col items-center py-6 pb-24">
      <div className="w-[95%] max-w-5xl bg-white rounded-3xl shadow-xl overflow-hidden flex flex-col md:flex-row">
        ...
        <aside className="md:w-1/3 bg-blue-600 text-white p-8 flex flex-col items-center justify-center text-center relative">
          ...
          {}
          <div
            className={`mb-8 relative group ${isEditing ? "cursor-pointer" : ""}`}
        ...
        <main className="flex-1 p-8 md:p-12 bg-white">
            ...
            <div>
              ...
              <p className="text-gray-400 text-sm">
                {isEditing
                  ...
                  : "Visualize suas informações (clique em Editar para alterar)"}
              </p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 gap-6 max-w-2xl">
            ...
            <div className="space-y-2">
              ...
              <div
                className={`flex items-center rounded-xl px-4 py-3 border transition-all ${isEditing ? "bg-white border-blue-500 shadow-sm" : "bg-gray-50 border-transparent"}`}
              >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-2">
                ...
                <div
                  className={`flex items-center rounded-xl px-4 py-3 border transition-all ${isEditing ? "bg-white border-blue-500" : "bg-gray-50 border-transparent"}`}
                >
            ...
            <div className="space-y-2 mb-8">
              <label
                className={`text-xs font-bold uppercase tracking-wide ml-1 ${isEditing ? "text-red-400" : "text-gray-400"}`}
              >
                Alterar Senha {isEditing ? "" : "(Bloqueado)"}
              </label>
              <div
                className={`flex items-center border-2 rounded-xl px-4 py-3 transition-all ${isEditing ? "bg-white border-red-50 focus-within:border-red-400" : "bg-gray-50 border-transparent"}`}
              >
                <Lock
                  className={`w-5 h-5 mr-3 ${isEditing ? "text-red-300" : "text-gray-300"}`}
                />
                <input
                  name="senha"
                  type="password"
                  placeholder={
                    isEditing ? "Digite apenas se quiser trocar" : "********"
                  }
