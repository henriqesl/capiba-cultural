...
const JoinCaravanaPage = ({ onBack, onJoin }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        if (!accessCode) return alert("Insira o código.");
        if (!user?.id) return alert("Erro de usuário.");
                ...
                <form onSubmit={handleSubmit} className="p-6 flex flex-col gap-5">
                    <div>
                        ...
                        <div className="relative">
                            ...
                            <input 
                                ...
                                className="w-full pl-10 p-3 bg-gray-50 border border-gray-200 rounded-xl focus:outline-none font-mono tracking-widest uppercase"
                                maxLength={10}
                            />
                        </div>
                    </div>
                    ...
                    <div className="pt-4">
                        <button type="submit" disabled={loading} className="w-full bg-blue-600 text-white font-bold py-4 rounded-xl shadow-lg hover:bg-blue-700 transition-colors">
                            {loading ? "Entrando..." : "Entrar na Caravana"}
                        </button>
                    </div>
                </form>
            </div>
