...
const RankingPage = ({ competitionId }) => {
    ...
    const fetchRanking = async () => {
      ...
      try {
        ...
        if (id) {
          ...
          dadosOrdenados = response.data.sort(
            (a, b) => b.saldoMoedaCapiba - a.saldoMoedaCapiba,
          );
        }
  ...
  if (loading)
    return (
      <div className="p-10 text-center text-white">Carregando Ranking...</div>
    );
        ...
        {!id && currentUser && (
          <div className="mb-6 p-4 bg-blue-50 border border-blue-100 rounded-xl flex items-center justify-between shadow-sm">
            ...
            <span className="font-bold text-blue-600">
              {currentUser.saldoMoedaCapiba || 0} pts
            </span>
          </div>
        )}
